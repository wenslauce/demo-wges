...
const HeroSection = () => {
  ...
  const [videoBrightness, setVideoBrightness] = useState(0.5);
  ...
  // Update video brightness based on scroll
  useEffect(() => {
    const unsubscribe = scrollYProgress.onChange(value => {
      const newBrightness = 0.5 - (value * 0.2);
  ...
  return (
    <section ref={heroRef} className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <motion.div 
        className="absolute inset-0 bg-gradient-to-b from-secondary/60 to-secondary/90 z-10"
        animate={{
          opacity: [0.8, 0.85, 0.8],
          background: [
            "linear-gradient(to bottom, rgba(23, 37, 84, 0.6), rgba(23, 37, 84, 0.9))",
            "linear-gradient(to bottom, rgba(23, 37, 84, 0.65), rgba(23, 37, 84, 0.9))",
            "linear-gradient(to bottom, rgba(23, 37, 84, 0.6), rgba(23, 37, 84, 0.9))"
          ]
        }}
        transition={{
